...
def total_revenue(data: dict, financial_index):
    ...
    if financial_index < len(financials):
        ...
        return float(line_items.get("netRevenue", 0))
    return 0.0
...
def total_borrowing(data: dict, financial_index):
    ...
    if financial_index < len(financials):
        ...
        long_term_borrowings = float(line_items.get("longTermBorrowings", 0))
        short_term_borrowings = float(line_items.get("shortTermBorrowings", 0))
...
def iscr(data: dict, financial_index):
    ...
    if financial_index < len(financials):
        pnl = financials[financial_index].get("pnl", {})
        line_items = pnl.get("lineItems", {})
        pbit = float(line_items.get("profitBeforeInterestAndTax", 0))
        depreciation = float(line_items.get("depreciation", 0))
        interest_expenses = float(line_items.get("interestExpenses", 0))
        
        numerator = pbit + depreciation + 1
